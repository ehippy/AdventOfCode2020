...
for line in fuckin_pws:
    parts = line.split(": ")
    pw = parts[1]
    rules_parts = parts[0].split(" ")
    sought_letter = rules_parts[1]
    bounds = rules_parts[0].split("-")
    position_letter_wanted = int(bounds[0])-1
    position_letter_banned = int(bounds[1])-1

    try:
        if (pw[position_letter_wanted] == sought_letter) != (pw[position_letter_banned] == sought_letter):
            good_passwords += 1
            print("Good password: %s" % line)
        else: 
            print("Bad password: %s" % line)
    except:
        pass

print("Good passwords: %s" % good_passwords)
